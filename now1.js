        ...
        .then(data => {
            ...
            if (data && data.records && Array.isArray(data.records.Station)) {
                ...
                if (numberOfRecords > 0) {
                    ...
                    sendmsg('api_data1', `最高溫: ${wadData.DailyExtreme.DailyHigh.TemperatureInfo.AirTemperature} 度 (${wadData.DailyExtreme.DailyHigh.TemperatureInfo.Occurred_at.DateTime.replace('T', ' ').substring(11, 19)})`);
                    sendmsg('api_data1', `最低溫: ${wadData.DailyExtreme.DailyLow.TemperatureInfo.AirTemperature} 度 (${wadData.DailyExtreme.DailyLow.TemperatureInfo.Occurred_at.DateTime.replace('T', ' ').substring(11, 19)})`);
                    sendmsg('api_data1', `觀測時間: ${staData.ObsTime.DateTime.replace('T', ' ').substring(0, 19)}`);
                    sendmsg('api_data1', ' ');
